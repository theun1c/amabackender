...
migrate-action:
    ...
    docker compose run --rm app-posegres-migrate \
		-path /migrations \
		-database postgres://${POSTGRES_USER}:${POSTGRES_PASSWORD}@app-posegres:5432/${POSTGRES_DB}?sslmode=disable \
		"$(action)"
...
migrate-down:
   	@make migrate-action action=dowm
